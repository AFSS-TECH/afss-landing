...
--
-- owner_id has no FK to auth.users on purpose: the reference seed block at the
-- bottom of this file inserts rows under a fixed placeholder UUID that isn't a
-- real auth user, so a hard FK would reject that insert.
--
...
-- Seeded under a fixed reference owner so you can verify the schema/data in the
-- table editor after running this script. The LIVE app does NOT read these rows —
-- each visitor gets their own isolated copy of this same dataset, auto-seeded by
-- js/data.js (seedDefaults()) on their first anonymous sign-in, scoped to their own
-- auth.uid() via RLS. This block just keeps a durable, query-able copy in sync.
do $$
declare
  ref_owner uuid := '00000000-0000-0000-0000-000000000000';
begin
  delete from erp_demo_projects where owner_id = ref_owner;
  insert into erp_demo_projects (owner_id, id, name, client, kontrak, hpp, progress, start, deadline, status, pm, workers, lokasi) values
  ...
  delete from erp_demo_employees where owner_id = ref_owner;
  insert into erp_demo_employees (owner_id, id, name, role, project, upah, initials, color, phone, status) values
  ...
  delete from erp_demo_attendance where owner_id = ref_owner;
  insert into erp_demo_attendance (owner_id, id, emp_id, name, role, project, project_id, checkin, checkout, hours, upah, status, gps, date) values
  ...
  delete from erp_demo_payroll where owner_id = ref_owner;
  insert into erp_demo_payroll (owner_id, id, emp_id, name, role, project, project_id, days, upah, lembur, makan, transport, kasbon, potongan, status, initials, color) values
  ...
  delete from erp_demo_stock where owner_id = ref_owner;
  insert into erp_demo_stock (owner_id, id, nama, kategori, qty, min, satuan, lokasi, harga) values
  ...
  delete from erp_demo_equipment where owner_id = ref_owner;
  insert into erp_demo_equipment (owner_id, id, name, total, dipinjam, kondisi, biaya_hari, project) values
  ...
  delete from erp_demo_purchase_requests where owner_id = ref_owner;
  insert into erp_demo_purchase_requests (owner_id, id, project_id, project, item, qty, satuan, est_price, requester, date, status, urgency) values
  ...
  delete from erp_demo_purchase_orders where owner_id = ref_owner;
  insert into erp_demo_purchase_orders (owner_id, id, vendor, item, total, date, status, due, project) values
  ...
  delete from erp_demo_gl where owner_id = ref_owner;
  insert into erp_demo_gl (owner_id, id, date, kode, akun, ket, debit, kredit, project) values
  ...
  delete from erp_demo_cashflow where owner_id = ref_owner;
  insert into erp_demo_cashflow (owner_id, month, masuk, keluar) values
  ...
  delete from erp_demo_stages where owner_id = ref_owner;
  insert into erp_demo_stages (owner_id, id, project_id, name, description, "order", status, target_pct, actual_pct, start, deadline) values
  ...
  delete from erp_demo_doc_uploads where owner_id = ref_owner;
  insert into erp_demo_doc_uploads (owner_id, id, stage_id, project_id, title, caption, type, drive_id, thumb_url, uploaded_by, uploaded_at, kinerja_note) values
